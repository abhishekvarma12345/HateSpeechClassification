...
class DataTransformation:
    ...
    def imbalance_data_cleaning(self):
        try:
            ...
            logging.info(f"Exited the imbalance_data_cleaning function and returned imbalance data")
    ...
    def raw_data_cleaning(self):
        ...
        try:
            ...
            raw_data[self.data_transformation_config.CLASS].replace([0, 1, 2], [1, 1, 0], inplace=True)

            # Let's change the name of the 'class' to label
            raw_data.rename(columns={self.data_transformation_config.CLASS:self.data_transformation_config.LABEL},inplace =True)
            logging.info(f"Exited the raw_data_cleaning function and returned the raw_data")
            return raw_data
    ...
    def concat_dataframe(self):
        ...
        try:
            ...
            df = pd.concat(frame)
            logging.info(f"returned the concatenated dataframe")
            return df
